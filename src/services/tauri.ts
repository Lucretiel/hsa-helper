...
export async function saveMetadata(metadata: HsaMetadata): Promise<HsaMetadata> {
  return invoke<HsaMetadata>("save_metadata", { metadata });
}

export async function uploadReceipt(receiptId: string, data: Uint8Array): Promise<void> {
