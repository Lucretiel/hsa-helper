import type { HsaEvent, EventType } from "../services/types";
...
function getEmptyMessage(filters: FilterState | undefined, hasAnyEvents: boolean): { title: string; subtitle: string } {
  if (!hasAnyEvents) {
    return {
      title: "No events recorded yet.",
      subtitle: "Click \"Add Event\" to record an expense, withdrawal, or deposit.",
    };
  }

  // Has events but filters are hiding them
  const typeLabel = filters?.eventTypes && filters.eventTypes.length > 0
...
export function EventList({ events, filters, hasAnyEvents, onViewReceipt }: EventListProps) {
  if (events.length === 0) {
